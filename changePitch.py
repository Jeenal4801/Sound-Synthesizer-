...
def pitchChange():
    given=input("Please enter the path to your file: \n")
    fn_wav = os.path.join('..', 'data', 'B', given)
    y, sr = librosa.load(fn_wav, sr=16000) # y is a numpy array of the wav file, sr = sample rate
    print('Enter the number of steps by wich you want to change the pitch')
    steps = float(input())
    changed = librosa.effects.pitch_shift(y, sr, steps)
    sr2=changed.getframerate()
    final=input("Done! Please enter a path to save the file: \n")
    s.write(final,changed,sr2)
